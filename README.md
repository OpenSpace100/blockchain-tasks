...
| 时间 | 课件                                                         | 作业                                                         |
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| W1D1 | 1. [Github Code](https://github.com/xilibi2003/blockchain)<br />2. [PDF](https://drive.google.com/file/d/1kf1K225UT8FapUbT0AwwF1cc7D4-QCsw/view?usp=sharing)<br />3. [Video](https://meeting.tencent.com/v2/cloud-record/share?id=1e922724-a046-4b74-b843-92aac8210ab5&from=3)(pwd:cQZJ) | 1. [Task1](https://github.com/OpenSpace100/blockchain-tasks/tree/main/simple-blockchain-impl)<br />2. [模拟实现谁小区快链](https://decert.me/quests/ed2d8324-54b0-4b7a-9cee-5e97d3c30030) |
...
| W1D3 | [PDF](https://drive.google.com/file/d/10BeY5zdTOkZT7n3oQhhM9_ZhSoq8z2s-/view?usp=sharing)、 [Code](./solidity_sample_code) | 编写一个Bank 合约:<br/>• 通过 Metamask 向Bank合约存款(转账ETH)<br/>• 在Bank合约记录每个地址存款金额<br/>• 编写 Bank合约 withdraw(), 实现只有管理员提取出所有的 ETH<br/>• 用数组记录存款金额前 3 名 |
...
| W4D3 | [PDF](https://drive.google.com/file/d/1s1V9B9HWCsFJ3aWiSmm-omdel84R4M0-/view?usp=sharing) | 组合使用 Multicall (delegateCall) 、 默克尔树、erc20 permit 授权, 实现一个默克尔树优<br/>惠价格(100 Token)购买名单, 使用 Multicall. 调用 2 个方法:• 合约: AirdopMerkleNFTMarket:<br/>• permitPrePay : 完成预授权<br/>• claimNFT( ) : 验证白名单,如果通过,直接使用 permitPrePay 的授权,转入 100 token, 并转出 NFT |
| W4D4 |                                                              |                                                              |
| W4D5 |                                                              |                                                              |
| W5D1 | [PDF](https://drive.google.com/file/d/1w40dpGf3XRzVwh9-0viw7aCtXJp9A2JV/view?usp=sharing) | 实现一个简单的多签钱包, 功能:<br/>• 多签持有人可提交交易<br/>• 其他多签人确认交易(使用交易的方式确认即可)<br/>• 达到多签⻔槛、任何人都可以执行交易 |
...
| W8D1 | [DAO 治理](https://drive.google.com/file/d/15WkTrQbVk46G8gblxM1mwsdss6fJFGEH/view?usp=sharing) | 实现y一个可以可计票的 Token<br/>• 实现一个通过 DAO 管理资金的 Bank:<br/>• 管理员可以从Bank合约中提取资金withdraw()<br/>• 治理 Gov 合约作为管理员, Gov 合约使用 Token 投票进行治理<br/>• 通过发起提案从Bank合约资金 |
